# Inspectl Temporal Local Pipelines Design
...
- An `inspectl` CLI for inspection and resume
...
On `run(resume=True)` or `inspectl resume <run_id>`:
...
The operator CLI should be named `inspectl`.
...
- `inspectl inspect <run_id> --step=<step>`
...
- The operator CLI is named `inspectl`.
